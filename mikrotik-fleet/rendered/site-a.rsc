# mkt-site-a -- rendered from templates/router.rsc.j2, do not hand-edit.
...
/system identity set name="mkt-site-a"
...
/ip address add address=10.253.10.1/24 interface=vlan10-mgmt
/interface vlan add name=vlan20-users vlan-id=20 interface=bridge-lan
/ip address add address=10.253.20.1/24 interface=vlan20-users

# --- Loopback ---
/interface bridge add name=lo comment="loopback, never bridged to a real port"
/ip address add address=10.253.0.1/32 interface=lo

# --- SNMP ---
/snmp set enabled=yes contact="noc@example.invalid" location="site-a"
/snmp community set [find default=yes] name="{{SECRET:snmp_community:site-a}}" read-access=yes write-access=no
...
/interface wireguard add name=wg-mgmt listen-port=51820 private-key="{{SECRET:wg_privkey:site-a}}"
...
/ip address add address=10.99.0.2/24 interface=wg-mgmt
